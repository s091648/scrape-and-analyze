"""
Tests for OTel tracer helpers in src/infrastructure/observability/otel_tracing.py.
...
def test_get_tracer_returns_tracer_instance():
    from src.infrastructure.observability.otel_tracing import get_tracer
    tracer = get_tracer()
...
def test_shutdown_tracing_does_not_raise_when_no_provider(monkeypatch):
    import src.infrastructure.observability.otel_tracing as tracing_module
    monkeypatch.setattr(tracing_module, "_provider", None)
    from src.infrastructure.observability.otel_tracing import shutdown_tracing
