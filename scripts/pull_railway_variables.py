...
Prerequisites (task R35 order):
  ...
  3. secrets/railway-shared.tfvars has `railway_project_id` and
     secrets/github-shared.tfvars has every `service_id_<key>` (non-secret UUIDs).

Usage:
    python scripts/pull_railway_variables.py [--as-tfvars] [SERVICE_KEY ...]
    (no SERVICE_KEY args = every service in railway-services.json)
...
GITHUB_SHARED_TFVARS = TF_DIR / "secrets" / "github-shared.tfvars"    # service_id_*
RAILWAY_SHARED_TFVARS = TF_DIR / "secrets" / "railway-shared.tfvars"  # railway_project_id
...
def _emit_tfvars(result):
    ...
    lines = [
        ...
        "# Split these lines into secrets/railway-{shared,<env>}.tfvars (see the",
    ...
    for k in sorted(seen):
        ...
        if prod and stg and prod_vals == stg_vals and len(prod_vals) == 1:
            scope = "both envs, same value -> railway-shared.tfvars"
        elif prod and stg:
            scope = "both envs, DIFFERS per env -> railway-<env>.tfvars"
        elif prod:
            scope = "production only -> railway-production.tfvars"
        else:
            scope = "staging only -> railway-staging.tfvars"
...
def main():
    ...
    tfvars = {**_load_tfvars(GITHUB_SHARED_TFVARS), **_load_tfvars(RAILWAY_SHARED_TFVARS)}
    project_id = tfvars.get("railway_project_id")
    if not project_id:
        raise PullError(f"railway_project_id is empty in {RAILWAY_SHARED_TFVARS}")
    ...
    for key in service_keys:
        ...
        if not service_id:
            print(f"::warning:: service_id_{key} not set in {GITHUB_SHARED_TFVARS.name} -- skipping {key}", file=sys.stderr)
            continue
