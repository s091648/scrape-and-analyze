...
- **FR-016**: 問答功能對所有訪客開放，但系統必須依照使用者身份施加每日請求次數上限（RPD）：訪客每日最多 3 次、一般登入會員每日最多 50 次、管理員無限制。超過限制時，系統必須回傳明確的提示訊息（HTTP 429），而非靜默失敗。系統必須提供 `GET /chat/quota` 端點，讓前端查詢目前剩餘次數與上限，供 UI 顯示配額資訊。
