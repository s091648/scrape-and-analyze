...
- **FR-013**: Exactly one credential — a GitHub token scoped to manage repository secrets/variables, needed to let the IaC tool authenticate to GitHub in the first place — MUST remain outside the IaC tool's own management, created and stored manually. The system MUST document this credential as the sole standing manual exception rather than leave it implicit.
...
- **CI Credential Store**: The GitHub Actions repository secrets and variables that `ci.yml`/`release.yml` read (`secrets.*`/`vars.*`) — brought into the same declarative definition and Applied State as the hosting platform's resources (FR-012), except for the one bootstrap credential (FR-013) that authenticates the IaC tool to GitHub and therefore cannot be self-managed.
...
- Per the Clarifications above, IaC scope includes the GitHub Actions secrets/variables store as well as the hosting platform's resources; the one standing manual exception is the bootstrap GitHub credential used to authenticate the IaC tool to GitHub itself (FR-013).
