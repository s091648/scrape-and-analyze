...
| `T6-07a` | [AGENT] | Reusable `.github/workflows/railway-config.yml` (`workflow_call`, `mode`/`environment`, `railwayapp/config@v1`), structured like `terraform.yml`. `ci.yml`: `railway-config-plan` + `deploy-staging-railway-config` (staging, PR) next to the terraform jobs. `release.yml`: `railway-config-production` / `railway-config-test-staging` as `uses:` calls, `detect`-gated. Auth = existing `secrets.RAILWAY_TOKEN` (`gh_env_railway_token`, env-scoped project token — verified works with `railway config`); no new secret, no `github-ci.tf` change | ✅ done |
| `T6-07b` | [MAINTAINER] | Land a trivial `.railway/**` PR to confirm the `railway-config-plan` PR comment + `deploy-staging-railway-config` apply run green | ☐ pending |
| `T6-08a` | [AGENT] | **v2** non-secret literals → `.railway/constants.ts`, `plan`-verified. **DONE** — RAG dense/sparse/chunking (`dcab558d`); `APP_ENV` ctx-branch, GRAFANA LOKI/OTLP/PROMETHEUS/TEMPO url+user, `GRAFANA_URL`, `CONTACT_EMAIL`, `VECTOR_DB_SCHEMA`, `SWAGGER_TRY_IT_OUT_ENABLED`, `CHATBOT_MAX_TOKENS`, `SEARCH_AUTOCOMPLETE_MAX_QUERY_LEN`, `SEARCH_MIN_DOC_FREQ` (`7b64ccc9`). Staging plan clean; every production change verified `== live`. **Deferred**: per-service `UV_GROUP` (build-ARG entanglement — do with 08c or later). | ✅ done |
| `T6-08b` | [AGENT] | **v2** service refs — `DATABASE_URL`→`Postgres.env.DATABASE_URL`, `REDIS_URL`→`Redis.env.REDIS_URL`, `VECTOR_DB_{HOST,NAME,PASSWORD,PORT,USER}`→`Postgres.env.PG*` (`c272e964`). Zero infra change (already matched live both envs). **Deferred** — no SDK string-composition form: `CACHE_REDIS_URL` / `SEARCH_INDEX_REDIS_URL` (`${{Redis.REDIS_URL}}/N`), `RAG_SPARSE_ENDPOINT_URL` / `CHAT_SERVICE_URL` / `BACKEND_URL` (`http://${{<svc>.RAILWAY_PRIVATE_DOMAIN}}:PORT`). | ✅ done (partial) |
| `T6-08c` | [AGENT] | **v2** secrets/refs → `process.env.X` via `need()` + `scripts/tfvars_to_env.py` (name map from `railway-services.json`), run by `railway-config.yml` (materialize tfvars from `TF_TFVARS_*` → explode into `$GITHUB_ENV`) before the `railwayapp/config` steps (`b26698aa`). ~24 vars/env. `CACHE_REDIS_URL`/`SEARCH_INDEX_REDIS_URL` + per-service `UV_GROUP` → literals in `constants.ts`. Verified: staging `plan` clean; production 146 changes, **all `severity:safe` / `variable.set` / non-destructive**. | ✅ done |
| `T6-08d` | [MAINTAINER] | `railway config apply` to absorb the accumulated T6-08a/b/c `preserve()→literal`/`→ref` transitions — staging auto via `deploy-staging-railway-config` on the next `.railway/**` PR; production via `release.yml`'s `railway-config-production` on the next `v*` tag (or by hand). Until then production `plan` shows 146 `severity: safe` pending. | ☐ pending |
| `T6-08e` | [AGENT] + [MAINTAINER] | Production drift for `NEXTAUTH_SECRET` / `GITHUB_PACKAGE_TOKEN` / `FRONTEND_ORIGIN` / `NEXTAUTH_URL`. **Decision (confirmed): the tfvars values are canonical** — [AGENT] removed `PROD_DRIFT_HOLD`, all 4 now `need()` (`60e9de41`). production `plan` = 155, all `severity:safe`. **[MAINTAINER]**: `NEXTAUTH_SECRET` change logs out every prod session on apply — schedule accordingly; confirm the `ghp_` token in the tfvars is valid before the prod apply. | ◑ code done |
| `T6-09` | [AGENT] | Retire the old Railway-vars path (`a988a2dd`): deleted `push_railway_variables.py`, `pull_railway_variables.py`, `split_tfvars.py`, `src/railway-*.toml`; dropped the 3 `*-railway-variables` Makefile targets + `terraform.yml`'s `railway variables` steps; updated `CLAUDE.md` / both READMEs / `variables.tf` / `versions.tf` / `.env.example`. **`railway-services.json` RETAINED** — now only `tfvars_to_env.py`'s name-map + `generate_terraform_docs.py`'s source (not the var-routing authority; `railway.ts` is). | ✅ done |
| `T6-10` | [AGENT] | `src/railway-scrape-and-analyze.toml` (+ the other 4 `src/railway-*.toml`) — deleted in T6-09 (`a988a2dd`); the staged TOML-syntax fix is moot. | ✅ done |
