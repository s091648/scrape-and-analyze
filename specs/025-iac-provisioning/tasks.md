...
| `T6-08c` | [AGENT] + [MAINTAINER] | **v2** secrets — remaining `preserve()`s → `process.env.X`. **Decided**: a step in `railway-config.yml` (`scripts/tfvars_to_env.py` or similar) explodes the existing base64 `TF_TFVARS_RAILWAY_{SHARED,<env>}` GH secrets into `$GITHUB_ENV` before the `railwayapp/config` apply step. Also pick up the 08a/08b deferred interpolated refs (plain `process.env`-composed strings) + `UV_GROUP`. `plan`-verified. | ☐ pending |
| `T6-08d` | [MAINTAINER] | `railway config apply` to absorb the accumulated T6-08a/b `preserve()→literal`/`→ref` transitions — staging auto via `deploy-staging-railway-config` on the next `.railway/**` PR; production via `release.yml`'s `railway-config-production` on the next `v*` tag (or by hand). Until then production `plan` shows ~101 `severity: safe` pending. | ☐ pending |
