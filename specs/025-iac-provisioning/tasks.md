...
| `T6-08e` | [MAINTAINER] | **Reconcile production drift** surfaced by T6-08c (held `preserve()` in `railway.ts` as `PROD_DRIFT_HOLD`): `NEXTAUTH_SECRET` (prod dashboard-backend/-frontend = literal `"generate-with-openssl-rand-base64-32"` placeholder — likely broken); `GITHUB_PACKAGE_TOKEN` (prod ≠ tfvars `ghp_` token — which is current?); `FRONTEND_ORIGIN` / `NEXTAUTH_URL` (prod = bare host `dashboard-frontend-production-fb6b.up.railway.app`, no `https://`, no `${{ref}}`). Pick the canonical value per var, set it (Railway or tfvars), then un-hold in `railway.ts`. | ☐ pending |
