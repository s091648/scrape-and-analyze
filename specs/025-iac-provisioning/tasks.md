...
| `T6-08c` | [AGENT] + [MAINTAINER] | **v2** secrets — remaining `preserve()`s → `process.env.X`. **Decided**: a CI step (`scripts/tfvars_to_env.py` or similar) explodes the existing base64 `TF_TFVARS_RAILWAY_{SHARED,<env>}` GH secrets into `$GITHUB_ENV` before the `railwayapp/config` apply steps in `railway.yml` / `release.yml`. Also pick up the 08a/08b deferred interpolated refs (they can be plain `process.env`-composed strings) + `UV_GROUP`. `plan`-verified. | ☐ pending |
| `T6-08d` | [MAINTAINER] | Run `railway config apply` (staging auto via CI once `RAILWAY_CONFIG_TOKEN` is set; production by hand) to absorb the accumulated T6-08a/b `preserve()→literal`/`→ref` transitions. Until then production `plan` shows ~101 `severity: safe` pending. | ☐ pending |
