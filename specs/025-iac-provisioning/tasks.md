...
| `T6-08a` | [AGENT] | **v2** non-secret literals — de-`preserve()` into `.railway/constants.ts`, `plan`-verified. **RAG group done** (commit `dcab558d`): 13 dense/sparse/chunking values; staging plan clean, production 46 `severity: safe` `preserve()→literal` (same value, verified live) awaiting one `railway config apply`. **Remaining groups**: GRAFANA URLs/users (not the keys), `APP_ENV` (ctx-branch), `SEARCH_*`, `VECTOR_DB_SCHEMA`, `SWAGGER_TRY_IT_OUT_ENABLED`, `CHATBOT_MAX_TOKENS`, per-service `UV_GROUP`, `CONTACT_EMAIL`. | ◑ in progress |
