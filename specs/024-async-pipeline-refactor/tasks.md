...
## Phase 9: User Story 6 - Every article's RAG embedding succeeds regardless of its size or how many other articles are ingesting concurrently (Priority: P2)

**Goal**: Concurrent articles' RAG embedding calls are coordinated through a single point per run instead of colliding independently on the shared embedding rate limit — no article's RAG ingestion fails purely because of how many other articles were ingesting concurrently or how many chunks it produced. See research.md item 11 for the root-cause trace and contracts/embedding-batch-coordinator-port.md for the interface this phase implements.

**Independent Test**: Run RAG ingestion concurrently for several articles (including one whose chunk count far exceeds `embed_batch_size`) against a rate limit low enough to force contention absent this fix; confirm embedding calls are coordinated through one point and articles are not lost to collision-driven rate-limit exhaustion.

**Note on repo boundary**: T072-T075 are implemented in the `chatbot-plugin-sdk` repository (a separate git repo, pinned via `pyproject.toml`'s `tag =`); T076-T083 are implemented in this repo (`scrape-analyzer`). `chatbot-plugin-sdk`'s own version bump/git tag and this repo's `pyproject.toml` pin update are **out of scope** for this task list — that repo's own CI/release process, per spec.md Assumptions.

### Implementation for User Story 6

- [X] T072 [US6] Implement `EmbeddingBatchCoordinator` + `EmbedWorkItem` in `chatbot_plugin_sdk/batching.py` (chatbot-plugin-sdk repo) — lazily-started single worker task draining a `queue_factory`-supplied `asyncio.Queue`, `embed_many(texts) -> list[list[float]]`, `aclose()`, per contracts/embedding-batch-coordinator-port.md. **Bug found and fixed during implementation**: the original `zip(batch, vectors)` result-assignment silently dropped items when a provider returned fewer vectors than texts, leaving their futures unresolved and hanging `embed_many()` forever — reproduced deterministically by chatbot-plugin-sdk's own pre-existing `test_raises_on_dense_vector_count_mismatch` test (full SDK suite stalled at that exact test on two separate runs). Fixed by validating `len(vectors) == len(batch)` and raising `EmbeddingError` (resolved onto every item in the batch) before ever calling `zip()` — see research.md item 11's final bullet and the contract doc's "Behavioral guarantees".
- [X] T073 [US6] Export `EmbeddingBatchCoordinator` (and the `QueueFactory` type alias) from `chatbot_plugin_sdk/__init__.py` (chatbot-plugin-sdk repo), alongside `SlidingWindowStrategy`/`EndpointProvider`
- [X] T074 [US6] Wire `IngestProcessor.configure()`'s new `embed_queue_factory` parameter through to a per-instance `EmbeddingBatchCoordinator`; route `_embed_in_batches_dense()` through `coordinator.embed_many()` instead of its own sequential batching loop; add `IngestProcessor.aclose()` delegating to the coordinator's `aclose()` — in `chatbot_plugin_sdk/processors/ingest.py` (chatbot-plugin-sdk repo). `_embed_in_batches_sparse()` unchanged (no observed rate-limit contention — `RAG_SPARSE_RPM` unset)
- [X] T075 [P] [US6] Unit tests (chatbot-plugin-sdk repo, `src/tests/test_batching.py` + additions to `src/tests/test_ingest.py`): `embed_many()` calls from multiple concurrent callers share one worker and may compose into mixed batches; a batch failure (including a vector-count mismatch, T072's fix) propagates to every `EmbedWorkItem` in that batch; a custom `queue_factory` is honored; single-caller behavior matches the pre-coordinator sequential loop; `aclose()` cancels the worker cleanly. Full SDK suite: 170 passed.
- [X] T072b (follow-up, requested post-implementation) Add `get_queue()`/`set_queue()` to `EmbeddingBatchCoordinator` and passthrough `get_embed_queue()`/`set_embed_queue()` to `IngestProcessor` (chatbot-plugin-sdk repo) — observability/test-seam accessors, acknowledged YAGNI, no production caller needs runtime queue replacement today. `set_queue()` migrates not-yet-claimed items and restarts the worker; items in the worker's in-flight batch are cancelled, not migrated (documented limitation). **Second latent hang found and fixed while adding this**: `_worker_loop()`'s cancellation handling only ever drained/cancelled the *queue's* leftover items, never the in-flight batch it had already claimed and was awaiting `dense.embed()` for — that batch's futures were left permanently unresolved (not even cancelled) on cancellation, silently hanging its caller forever. This affects `aclose()` too (fixed as part of the same change), though it never manifested there since `aclose()` is only called after every `embed_many()` call has already settled in every current call site. New tests in `src/tests/test_batching.py::TestGetSetQueue` and `src/tests/test_ingest.py::TestGetSetEmbedQueue` (using `asyncio.Event`-based synchronization, not `sleep(0)` counting, to deterministically reach "worker mid-embed()" / "item queued but unclaimed" states). Full SDK suite: 180 passed. scrape-analyzer full suite re-verified unaffected: 1217 passed.
- [X] T076 [US6] Replace `asyncio.to_thread(use_case.execute, article)` + thread-bounded `asyncio.Semaphore` in `src/entrypoints/cli/backfill_rag.py`'s `_backfill_one`/`_backfill_all` with `AsyncIngestArticleForRagUseCase.execute()` awaited directly under a plain `asyncio.Semaphore`, all on the main event loop (research.md item 11 — the coordinator's queue/worker require every concurrent `ingest()` call to share one event loop). Existing `src/tests/unit/entrypoints/cli/test_backfill_rag.py` mocks updated from `MagicMock` to `AsyncMock` for `use_case.execute`/`.aclose()` to match the new async contract; `test_concurrency_flag_bounds_in_flight_execute_calls` switched from a blocking `time.sleep` side effect (meaningless once execute() runs on the main loop, not a worker thread) to `await asyncio.sleep()`.
- [X] T077 [US6] In `src/bootstrap.py`, change `build_rag_backfill_pipeline()` to call `build_async_rag_ingestion_service()` instead of `build_rag_ingestion_service()`, wiring `AsyncIngestArticleForRagUseCase` in place of the sync `IngestArticleForRagUseCase`
- [X] T078 [US6] Call `processor.aclose()` at the end of `CollectionPipeline.run()` (after Barrier 2 resolves). Landed as a `rag_service_aclose: Optional[Callable[[], Awaitable[None]]]` constructor param on `CollectionPipeline` (not the raw `IngestProcessor` — kept the existing `AsyncRagIngestionService` Protocol boundary intact) — `bootstrap.py` passes `_rag_ingestion_service.aclose` when RAG is enabled. `AsyncRagIngestionService` Protocol, `AsyncRagSdkIngestionService`, and `AsyncIngestArticleForRagUseCase` each gained a passthrough `aclose()` to reach `IngestProcessor.aclose()` through that same boundary.
- [X] T079 [US6] Call `processor.aclose()` at the end of `src/entrypoints/cli/backfill_rag.py`'s run. Landed as a new `_run_backfill()` wrapper (`_backfill_all()` inside a `try`/`finally` calling `use_case.aclose()`) instead of `main()`'s outer `finally` block — the coordinator's worker task is bound to the event loop it was created on, and `main()`'s outer `finally` runs *after* `asyncio.run()` has already closed that loop, so `aclose()` has to happen inside the same `asyncio.run()` call as the ingestion work, not after it.
- [X] T080 [US6] Delete `scripts/backfill_rag_embeddings.py` (dead code — bypasses `build_dense_provider()`'s rate limiting, calls `ingest()` without `await`, passes `url` under the wrong kwarg; superseded by `src/entrypoints/cli/backfill_rag.py`)
- [X] T081 [US6] Remove the `backfill-rag`, `backfill-rag-dry-run`, `backfill-rag-remote`, `backfill-rag-remote-production` targets and their `.PHONY` entries from `Makefile` — none replaced (the active entrypoint has no `make` target, runs only as a Railway Cron Service)

### Tests for User Story 6

- [X] T082 [P] [US6] Integration test: concurrent RAG ingestion for several articles — including one with far more chunks than `embed_batch_size` — against a low rate limit completes for every article that would also have succeeded serially (no collision-driven `RateLimitExhausted`), in `src/tests/integration/test_rag_embedding_coordination.py`. Landed as 3 tests using a fake in-memory `DenseEmbeddingProvider` (records call sizes, no real network) through the real `IngestProcessor`/`AsyncRagSdkIngestionService`/`AsyncIngestArticleForRagUseCase` stack — matches this repo's existing `@pytest.mark.integration` convention for RAG tests (mocked processor/provider, no real DB/API — see `test_rag_ingestion_pipeline.py`).
- [X] T083 [P] [US6] Integration/unit test: `backfill_rag.py`'s concurrent article ingestion (bounded by `--concurrency`) shares one `EmbeddingBatchCoordinator`/rate limiter instance across all concurrently-ingesting articles in a run, in `src/tests/integration/test_backfill_rag_concurrency.py`. Landed as 2 tests calling `_run_backfill()` directly against a real `IngestProcessor` with a fake dense provider — asserts `processor._dense_coordinator._worker_task is None` after the run (proves `aclose()` reached the shared coordinator, since it only clears that reference on a successful close).

**Full suite status after Phase 9**: chatbot-plugin-sdk: 170 passed. scrape-analyzer: 1217 passed (1082 unit + 135 integration, 6 deselected), zero new warnings beyond the pre-existing `jieba` deprecation notice. Verified against the local (uncommitted) chatbot-plugin-sdk changes via the repo's existing `../chatbot-plugin-sdk:/opt/chatbot-plugin-sdk` + `uv pip install -e` LOCAL DEV ONLY pattern (temporarily added to `test_service` in `docker-compose.yml` for this verification, then reverted — that pattern already existed on `app`/`chatbot_plugin`).

- [X] T072c (follow-up, requested post-implementation) Add `RAG_DISPATCH_CONCURRENCY` env var (`src/config/settings.py`, default 10) and wire a `BoundedSemaphore` gating `CollectionPipeline._run_rag_ingestion()`'s body (opening the `AsyncSession`, not `_dispatch_rag()`'s task creation) — bounds real, unpooled Postgres connections (`NullPool`, `get_async_sessionmaker()`), not embedding-API throughput. `build_collection_pipeline()` threads it through as `CollectionPipeline(rag_dispatch_concurrency=RAG_DISPATCH_CONCURRENCY, ...)`. New tests in `src/tests/unit/test_collection_pipeline_rag_dispatch_concurrency.py` (concurrency actually bounded; `_dispatch_rag()` itself never blocks on the semaphore — FR-002; default matches settings). `.env`/`.env.example` updated. chatbot-plugin-sdk repo untouched — scrape-analyzer-only change. Full suite: 1220 passed (after rebuilding the `test_service` Docker image, which had cached the previous `chatbot-plugin-sdk` git-tag pin from before it was bumped to v1.2.0).

**Checkpoint**: User Story 6 complete — RAG ingestion (live pipeline and backfill) no longer loses articles to rate-limit contention introduced by User Story 1's concurrency, matching spec.md FR-014 through FR-016 and SC-007.
...
- **User Story 6 (Phase 9)**: Depends on Foundational (T028's `build_async_rag_ingestion_service()`/`AsyncIngestArticleForRagUseCase` from Phase 2f) and, for the SDK-side work (T072-T075), nothing else in this repo — those tasks land in `chatbot-plugin-sdk` and can be done independently of every other phase. The scrape-analyzer-side tasks (T076-T081) depend on T072-T075 being available (a new SDK version with the coordinator). Independent of User Stories 1-5's own work beyond that shared foundational builder — added after Phase 8 as a correctness fix for a contention pattern Phases 1-8's own concurrency introduced.
...
7. + User Story 6 (added post-Phase-8, as a correctness fix for a contention pattern User Story 1's own concurrency exposed) → RAG ingestion no longer loses articles to shared-rate-limit collisions (SC-007).
