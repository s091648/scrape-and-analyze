...
- **Single-caller behavior is unchanged from before this coordinator existed**: a consumer that never has two `embed_many()`/`ingest()` calls in flight at once observes the same effective batching (sequential, `embed_batch_size`-sized calls to `dense.embed()`) as `IngestProcessor`'s previous inline loop — the coordinator adds one queue/future hop, not a behavior change, for that case.

## Non-goals

- No cross-process coordination — the coordinator's queue and worker are in-process, in-event-loop constructs (`asyncio.Queue`/`asyncio.Task`), matching this feature's existing single-process/single-event-loop constraint (plan.md Technical Context Constraints).
