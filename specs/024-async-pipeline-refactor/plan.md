...
- `chatbot_plugin_sdk` — originally assessed as needing no changes (`IngestProcessor.ingest()` is already `async def`, and its dense/sparse embedding providers are already awaited internally). **Revised for User Story 6** (research.md item 11): concurrent `ingest()` calls across articles independently collide on one shared per-run rate limiter, so the SDK gains a new `EmbeddingBatchCoordinator` that `IngestProcessor` routes dense-embedding batching through internally — `ingest()`'s public signature and single-caller behavior are unchanged. This feature adds a new sibling builder, `build_async_rag_ingestion_service()`, using the already-existing `AsyncPgBackend` — `build_rag_ingestion_service()` (sync, `SyncPgBackend`) stays as the function the RAG-backfill job's builder calls, but **as of User Story 6, `build_rag_backfill_pipeline()` itself switches to calling `build_async_rag_ingestion_service()` instead** (research.md item 11) — the coordinator requires every concurrent `ingest()` call to run in one event loop, which the backfill job's previous `asyncio.to_thread`-per-article model didn't provide. See research.md items 3 and 11.
...
│   └── embedding-batch-coordinator-port.md   # User Story 6 addendum
