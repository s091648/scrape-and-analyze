...
interface TagsPageContentProps {
  /** Server-rendered tag groups for the topic the server resolved, seeded from
   * `app/tags/page.tsx`'s SSR fetch. `undefined` when the server didn't fetch (no session —
   * spec.md User Story 3) or the fetch failed (FR-007). Trusted directly — `app/layout.tsx`
   * seeds TopicProvider from the exact same `cache()`-shared topic resolution `app/tags/page.tsx`
   * used to fetch this, so they can't disagree within one request. */
  initialGroups?: TagGroupOut[]
}

export default function TagsPageContent({ initialGroups }: TagsPageContentProps) {
  ...
  const hasSeed = !!initialGroups

  const [groups, setGroups] = useState<TagGroupOut[]>(hasSeed ? initialGroups! : [])
