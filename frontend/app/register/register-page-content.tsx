    ...
    <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center px-4">
      <div className="w-full max-w-sm space-y-8">
        <div className="text-center space-y-2">
          ...
          <p className="text-sm text-muted-foreground">New accounts are created with user role</p>
        </div>

        <div className="rounded-2xl border border-border bg-card p-6 space-y-4">
          {(error || alreadyRegistered) && (
            <div className="rounded-xl border border-destructive/20 bg-destructive/5 px-4 py-3 text-sm text-destructive">
              {alreadyRegistered ? 'This Google account is already registered. ' : error}
              {alreadyRegistered && (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-sm font-medium" htmlFor="name">Display name (optional)</label>
