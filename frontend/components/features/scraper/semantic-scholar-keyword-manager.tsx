...
interface SSKeyword { id: string; keyword: string }

export function SemanticScholarKeywordManager({
  keywords,
  onAdd,
  onDelete,
}: {
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
        {t('admin.semanticScholarKeywords')}
      </p>

      <div className="flex flex-wrap gap-2 min-h-6">
        {keywords.length === 0 && (
          <p className="text-xs text-muted-foreground italic">{t('admin.noSemanticScholarKeywords')}</p>
        )}
        {keywords.map(kw => (
      ...
      <div className="flex gap-2">
        <input
          ...
          placeholder={t('admin.semanticScholarKeywordPlaceholder')}
          value={value}
