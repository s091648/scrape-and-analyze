...
interface WeeklyReportTopicDialogProps {
  open: boolean
  onClose: () => void
  topicSpan: OtlpSpan
  stageSpans: SpanNode[]
...
function getLabelOverride(span: OtlpSpan, t: (k: string, p?: Record<string, string | number>) => string): string | undefined {
  if (span.name === 'weekly_report.translate') {
    const lang = getAttr(span, 'translation.language')
    if (lang) return t('admin.stageTranslateLabel', { lang: String(lang) })
  }
  if (span.name === 'weekly_report.notify') {
    const channel = getAttr(span, 'notify.channel')
    if (channel) return t('admin.weeklyReportNotifyLabel', { channel: String(channel) })
  }
  return undefined
}
...
export function WeeklyReportTopicDialog({
  open,
  onClose,
  topicSpan,
  stageSpans,
  highlightedSpanId,
}: WeeklyReportTopicDialogProps) {
  const { t } = useI18n()
  ...
  const totalMs = spanDurationMs(topicSpan)
      ...
      <DialogContent className="max-w-5xl w-full max-h-[90vh] flex flex-col">
        <DialogHeader>
          ...
          <p className="text-xs text-muted-foreground">
            ...
            {outcome && <> · {t('admin.stageAttr_weeklyReportOutcome')}: <span className="font-mono">{outcome}</span></>}
            {articleCount != null && <> · {t('admin.stageAttr_weeklyReportArticleCount')}: {articleCount}</>}
          </p>
        </DialogHeader>
          ...
          <div className="flex flex-col items-stretch">
            {stageSpans.length === 0 ? (
              <p className="text-sm text-muted-foreground py-4">
                {t('admin.weeklyReportTopicNoStages')}
              </p>
            ) : (
              topLevel.map((node, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <StageCard
                      ...
                      labelOverride={getLabelOverride(node.span, t)}
                    ...
                    {/* Child spans (e.g. translate/notify under the topic span) */}
