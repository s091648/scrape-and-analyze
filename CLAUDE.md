...
### Infrastructure (`infra/terraform/github/`)
...
| `make terraform-apply ENV=staging\|production` | Apply GitHub-side (needs `infra/terraform/github/.env` — template `.env.example` — + `secrets/*.tfvars`) |
...
2. **Pre-dedup** — Drop URLs already analyzed (via `UrlHash`). Applied twice: as a pre-fetch filter during discover, then again after fetch (`pipeline.dedup` span, which also collapses within-batch duplicate URLs)
3. **Fetch** — `ScrapeExecutor` runs concurrent fetches (fixed pool: 5 fetch workers, per-host `BoundedSemaphore(1)`, robots.txt respect for blogs)
...
**Concurrency knobs** — the fetch stage (step 3) uses a fixed thread pool (`ScrapeExecutor`: 5 fetch + 5 discover workers, `fetch_delay=5.0s`; **not** env-configurable). The per-article async stage (steps 4–6) is bounded by the `TEXT_STAGE_CONCURRENCY` (8) and `RAG_DISPATCH_CONCURRENCY` (4) `asyncio.BoundedSemaphore`s, kept at/under the async DB pool cap (`ASYNC_DB_POOL_SIZE + ASYNC_DB_MAX_OVERFLOW`, 20); `PIPELINE_EXECUTOR_MAX_WORKERS` (32) sizes the event loop's default executor (DNS / `asyncio.to_thread`). Everything except the `ScrapeExecutor` pool is env-overridable — see `src/config/settings.py`.

The scheduled runner (`src/entrypoints/cli/main.py`) bridges into this async pipeline via `asyncio.run()`, adds 0-180s random startup jitter (disable with `RUN_IMMEDIATELY=1`), and caps the pipeline run at a 50-min hard timeout via `asyncio.timeout(MAX_EXECUTION_TIME)` around `pipeline.run()` (on expiry the run is cancelled, the async engine is disposed, and final telemetry still flushes). No custom signal handlers are installed — SIGTERM terminates the process directly; the platform's SIGKILL is the backstop.

### Detached Side-Task Pattern (RAG, Translation)

RAG ingestion and translation are both "detached side-tasks" off the main per-article chain: dispatched when a specific event fires, run as their own `asyncio.Task` that the triggering handler never awaits, and collected by a later barrier instead of blocking the article's own text-stage completion. `translation_downstream_builder` in `src/bootstrap.py` is written to deliberately mirror `rag_downstream_builder` — same shape, so this is the template for adding a third one:

1. **A `xxx_downstream_builder(session)` closure in `bootstrap.py`**, called once per detached task with that task's own fresh `AsyncSession` (never the main per-article session, which may already be closed by the time a detached task actually runs). It builds the repos + Application-layer use case(s) bound to that session, a small private `AsyncInMemoryEventBus` subscribed only to that task's own `XxxFailedEvent` → `FailedTaskPersistenceHandler`, and returns one Handler object exposing `.handle(event, ...)`.
2. **A dedicated `asyncio.BoundedSemaphore` on `CollectionPipeline`** (mirrors `_rag_dispatch_semaphore` / `_translation_dispatch_semaphore`) gating how many of these detached tasks concurrently hold an open session — keep it, together with `TEXT_STAGE_CONCURRENCY` and every other side-task's semaphore, at or under the async DB pool cap.
3. **A `_run_xxx(event)` method**: `async with` that semaphore, `async with` a fresh session from `async_sessionmaker_factory`, build the handler via the closure, `await handler.handle(event)`.
4. **A `_dispatch_xxx(event)` method**: `asyncio.create_task(self._run_xxx(event))`, tracked in a collection for later settling — not awaited here.
5. **Subscribe `_dispatch_xxx` to whichever event should trigger it** (`ArticleProcessedEvent` for RAG, `AnalysisCompletedEvent` for translation), inside `_process_article_text` or `article_downstream_builder` depending on whether it needs to fire before or after the builder's own subscriptions.
6. **Decide whether it should block a barrier**: use `SettlingTaskGroup` (`src/infrastructure/shared/events/settling_task_group.py`) if this new task should be collected together with a repeatable dispatch/settle cycle within `pipeline.process_articles` (like translation's Barrier 1.5, which blocks `TextPipelineCompletedEvent`); use a plain `List[asyncio.Task]` + a one-time `asyncio.gather()` at run-end if it should only be collected once, after everything else (like RAG's Barrier 2, which blocks `PipelineCompletedEvent`). `SettlingTaskGroup` is the more reusable choice for a new side-task unless it genuinely only ever settles once per run.
...
**Provider config is DB-driven, not a TOML file** — there is no `providers.toml` in this repo; a prior file-based config was superseded by the `llm_providers` table (`models/llm_provider.py`) in migration `16_add_llm_providers.py`. `shared/llm_provider.py::load_active_providers()` / `load_active_embedding_providers()` / `load_active_multimodal_provider()` load active rows filtered by `type` (`'llm'` / `'embedding'` / `'multimodal'`) and `is_active=True`, ordered by `priority`. Two builders in `src/bootstrap.py` consume these directly from the DB on every run — `build_llm_service()` (sync `ResilientLLMService`, used by the weekly-report / release-notes / tag-embedding-backfill entrypoints) and `build_async_llm_service()` (async `AsyncResilientLLMService`, the `ProviderSelector`-aware version, used by `build_collection_pipeline()`). No config file, no redeploy needed to change a provider, model, or priority. Managed at runtime via `backend/routers/llm_providers.py` (full CRUD + `/reorder`, all `require_admin`) and the `/admin/llm-providers` dashboard page.
...
- Translation uses a parallel-table pattern: `AnalysisTranslation`, `TagTranslation`, `TagGroupTranslation` each have a `language` column + unique constraint on `(parent_id, language)`. English content was normalized into translations by migration `15_add_translations` (article-body translations came later, in `20_add_article_translation`).
...
- **Alembic migrations** in `alembic/versions/` — numbered prefix (00–26 and counting); early ones pair the number with a hex revision ID, later ones use descriptive strings like `"05_normalize_tags"`
- **Dependency management** — `uv` with `uv.lock`, `requires-python = ">=3.11"` (CI + containers run 3.11), dependency groups in `pyproject.toml`: core, scraper, backend, observability, dev
...
- **DB access & query patterns**
  - **ORM / Core first** — reads and DML go through the SQLAlchemy ORM or Core (`select()` / `insert()` / `update()` / `delete()`, `pg_insert(...).on_conflict_*()`, `.with_for_update()`, `.from_select()`). Raw `text()` SQL is reserved for what those genuinely can't express:
    - pgvector operators (`<=>`) and `CAST(:v AS vector/sparsevec)` — `search_service.py`, `tag_service.get_similar_groups`, the tag-group `embedding` writes in `routers/tags.py`.
    - `search_service.py`'s `core.articles` / `vectors.*` reads, which intentionally use **fully-qualified real-schema names** so the integration harness's `schema_translate_map` does *not* rewrite them (they must read the same rows the raw-SQL-seeded vector tables do — see `backend/tests/integration/test_search.py`'s module docstring). Everywhere else, unqualified raw SQL resolves via `search_path` to the same place translated ORM SQL does, so there's no reason not to use the ORM.
  - **No N+1 / eager-load explicitly** — never let a relationship lazy-load inside a loop. Pull what you need up front with `selectinload()` (collections) / `joinedload()` or `contains_eager()` (many-to-one). E.g. `graph_service.query_analyses` / `query_group_articles` eager-load `Analysis.article` + `Article.tags`; `article_service.get_tag_groups_for_article` uses `contains_eager(Tag.group_def)`.
  - **Batch translation lookups** — the parallel-table translations (`AnalysisTranslation`, `ArticleTranslation`, `TagTranslation`, `TagGroupTranslation`, `WeeklyReportTranslation`) are always fetched for a whole page of parent ids in one `.in_([...])` query and turned into a `{parent_id: row}` dict — never one query per row.
  - Relationship loaders are still the SQLAlchemy default (`lazy="select"`); moving the hot ones to `lazy="raise_on_sql"` so missing eager-loads fail loudly is a known follow-up, not yet done.
