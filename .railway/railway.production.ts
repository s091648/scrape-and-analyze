...
export default defineRailway(() => {
  const scrapeAndAnalyze = github("s091648/scrape-and-analyze", { branch: "master", rootDirectory: "/" });
  const scrapeAndAnalyze2 = github("s091648/scrape-and-analyze", { branch: "master", rootDirectory: "/frontend" });
  const scrapeAndAnalyze3 = github("s091648/scrape-and-analyze", { rootDirectory: "/" });
  ...
  const redisVolumeT232 = volume("redis-volume-t232", { alerts: { usage: { "100": {}, "80": {}, "95": {} } }, allowOnlineResize: true, region: "asia-southeast1-eqsg3a", sizeMB: 5000 });
  ...
  const dedup_reconcile = service("dedup_reconcile", {
    ...
    replicas: { "asia-southeast1-eqsg3a": 1 },
    deploy: { cronSchedule: "0 20 * * *", restartPolicyType: "NEVER" },
    networking: { privateNetworkEndpoint: "dedupreconcile" },
    env: { APP_ENV: preserve(), CONTACT_EMAIL: preserve(), DATABASE_URL: preserve(), FIXIE_URL: preserve(), GRAFANA_API_KEY: preserve(), GRAFANA_LOKI_URL: preserve(), GRAFANA_LOKI_USER: preserve(), GRAFANA_OTLP_ENDPOINT: preserve(), GRAFANA_OTLP_USER: preserve(), SENTRY_DSN: preserve(), TELEGRAM_BOT_TOKEN: preserve(), TELEGRAM_CHAT_ID: preserve(), UV_GROUP: preserve() },
  });
  ...
  const scrapeAndAnalyze = service("scrape-and-analyze", {
    ...
    build: { buildEnvironment: "V3", builder: "DOCKERFILE", dockerfilePath: "/src/Dockerfile" },
    replicas: { "asia-southeast1-eqsg3a": 1 },
    deploy: { cronSchedule: "0 8 * * *", restartPolicyType: "NEVER" },
    env: { APP_ENV: preserve(), CACHE_REDIS_URL: preserve(), CONTACT_EMAIL: preserve(), DATABASE_URL: preserve(), FIXIE_URL: preserve(), GEMINI_API_KEY: preserve(), GITHUB_PACKAGE_TOKEN: preserve(), GRAFANA_API_KEY: preserve(), GRAFANA_LOKI_URL: preserve(), GRAFANA_LOKI_USER: preserve(), GRAFANA_OTLP_ENDPOINT: preserve(), GRAFANA_OTLP_USER: preserve(), OPENROUTER_API_KEY: preserve(), RAG_CHUNK_OVERLAP: preserve(), RAG_CHUNK_SIZE: preserve(), RAG_DENSE_API_KEY_ENV: preserve(), RAG_DENSE_DIMENSION: preserve(), RAG_DENSE_ENDPOINT_URL: preserve(), RAG_DENSE_MODEL: preserve(), RAG_DENSE_PROVIDER: preserve(), RAG_DENSE_RPD: preserve(), RAG_DENSE_RPM: preserve(), RAG_DENSE_TPM: preserve(), RAG_EMBED_BATCH_SIZE: preserve(), RAG_GEMINI_API_KEY: preserve(), RAG_SPARSE_DIMENSION: preserve(), RAG_SPARSE_ENDPOINT_URL: preserve(), RAG_SPARSE_MODEL: preserve(), RAG_SPARSE_PROVIDER: preserve(), RAG_SPARSE_RPD: preserve(), RAG_SPARSE_RPM: preserve(), RAG_SPARSE_TPM: preserve(), SENTRY_DSN: preserve(), TELEGRAM_BOT_TOKEN: preserve(), TELEGRAM_CHAT_ID: preserve(), VECTOR_DB_HOST: preserve(), VECTOR_DB_NAME: preserve(), VECTOR_DB_PASSWORD: preserve(), VECTOR_DB_PORT: preserve(), VECTOR_DB_SCHEMA: preserve(), VECTOR_DB_USER: preserve() },
  });
  ...
  const backfill_rag = service("backfill_rag", {
    source: scrapeAndAnalyze3,
    build: { buildEnvironment: "V3", builder: "DOCKERFILE", dockerfilePath: "/src/Dockerfile" },
    start: "/app/.venv/bin/python -m src.entrypoints.cli.backfill_rag",
    replicas: { "asia-southeast1-eqsg3a": 1 },
    deploy: { cronSchedule: "0 20 * * *", restartPolicyType: "NEVER" },
    networking: { privateNetworkEndpoint: "ragbackfill" },
    env: { APP_ENV: preserve(), CONTACT_EMAIL: preserve(), DATABASE_URL: preserve(), FIXIE_URL: preserve(), GITHUB_PACKAGE_TOKEN: preserve(), GRAFANA_API_KEY: preserve(), GRAFANA_LOKI_URL: preserve(), GRAFANA_LOKI_USER: preserve(), GRAFANA_OTLP_ENDPOINT: preserve(), GRAFANA_OTLP_USER: preserve(), RAG_CHUNK_OVERLAP: preserve(), RAG_CHUNK_SIZE: preserve(), RAG_DENSE_API_KEY_ENV: preserve(), RAG_DENSE_DIMENSION: preserve(), RAG_DENSE_ENDPOINT_URL: preserve(), RAG_DENSE_MODEL: preserve(), RAG_DENSE_PROVIDER: preserve(), RAG_DENSE_RPD: preserve(), RAG_DENSE_RPM: preserve(), RAG_DENSE_TPM: preserve(), RAG_EMBED_BATCH_SIZE: preserve(), RAG_GEMINI_API_KEY: preserve(), RAG_SPARSE_DIMENSION: preserve(), RAG_SPARSE_ENDPOINT_URL: preserve(), RAG_SPARSE_MODEL: preserve(), RAG_SPARSE_PROVIDER: preserve(), RAG_SPARSE_RPD: preserve(), RAG_SPARSE_RPM: preserve(), RAG_SPARSE_TPM: preserve(), SENTRY_DSN: preserve(), TELEGRAM_BOT_TOKEN: preserve(), TELEGRAM_CHAT_ID: preserve(), UV_GROUP: preserve(), VECTOR_DB_HOST: preserve(), VECTOR_DB_NAME: preserve(), VECTOR_DB_PASSWORD: preserve(), VECTOR_DB_PORT: preserve(), VECTOR_DB_SCHEMA: preserve(), VECTOR_DB_USER: preserve() },
  });

  return project("scraper", {
    resources: [weeklyReport, dedup_reconcile, storybookUI, dashboardFrontend, scrapeAndAnalyze, refreshMetrics, Redis, Postgres, fastembed, dashboardBackend, chatbotPlugin, backfill_rag, redisVolumeT232, postgresVolume],
  });
});
