...
class AsyncSqlAlchemyAnalysisRepository(AsyncAnalysisRepository):
    ...
    async def save(self, analysis: Analysis) -> None:
        ...
        The whole body — including the initial flush(), not just commit() — is
        one rollback-and-reraise guard: this session is shared with
        FailedTaskPersistenceHandler (bootstrap.py's article_downstream_builder),
        so a flush() failure left un-rolled-back would leave the session unusable
        for that handler's own later commit(), silently losing the failure record
