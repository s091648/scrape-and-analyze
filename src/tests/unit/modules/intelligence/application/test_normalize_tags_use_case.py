...
@pytest.mark.asyncio
async def test_high_similarity_reuses_existing_tag_without_saving_new():
    ...
    await uc.execute(analysis_id=analysis_id, article_id=article_id,
...
@pytest.mark.asyncio
async def test_mid_similarity_saves_new_tag_and_creates_suggestion():
    uc, embed_svc, tag_repo = _make_use_case()
    analysis_id = uuid.uuid4()
    article_id = uuid.uuid4()
    existing_tag = TagData(id=uuid.uuid4(), name="real-time sync", tag_group_name="digital_twin")
...
@pytest.mark.asyncio
async def test_low_similarity_saves_new_tag_without_suggestion():
    uc, embed_svc, tag_repo = _make_use_case()
...
@pytest.mark.asyncio
async def test_empty_tag_name_is_skipped():
    uc, embed_svc, tag_repo = _make_use_case()
...
@pytest.mark.asyncio
async def test_auto_merge_emits_tag_auto_merged_log(mock_logger):
    uc, embed_svc, tag_repo = _make_use_case()
    existing_tag = TagData(id=uuid.uuid4(), name="real-time sync", tag_group_name="digital_twin")
...
@patch("src.modules.intelligence.application.use_cases.normalize_tags.logger")
@pytest.mark.asyncio
async def test_suggestion_emits_tag_suggestion_created_log(mock_logger):
...
@patch("src.modules.intelligence.application.use_cases.normalize_tags.logger")
@pytest.mark.asyncio
async def test_new_tag_emits_tag_created_log(mock_logger):
    uc, embed_svc, tag_repo = _make_use_case()
...
@pytest.mark.asyncio
async def test_rollback_on_exception_no_commit():
    uc, embed_svc, tag_repo = _make_use_case()
...
@pytest.mark.asyncio
async def test_rollback_on_repo_save_exception():
    uc, embed_svc, tag_repo = _make_use_case()
    embed_svc.embed_batch.return_value = [[0.1] * 768]
    tag_repo.find_similar.return_value = []
...
@pytest.mark.asyncio
async def test_embed_batch_called_once_with_all_tag_names():
    uc, embed_svc, tag_repo = _make_use_case()
    tag_repo.find_similar.return_value = []
    tag_repo.save.return_value = TagData(id=uuid.uuid4(), name="x", tag_group_name="g")
...
@pytest.mark.asyncio
async def test_exact_auto_merge_threshold_triggers_auto_merge():
    ...
    await uc.execute(analysis_id=uuid.uuid4(), article_id=article_id,
               tag_groups=[("g", ["tag a"])])
...
@pytest.mark.asyncio
async def test_exact_suggest_threshold_creates_suggestion():
    uc, embed_svc, tag_repo = _make_use_case(auto_merge=0.95, suggest=0.90)
    existing_tag = TagData(id=uuid.uuid4(), name="tag-a", tag_group_name="g")
...
@pytest.mark.asyncio
async def test_just_below_suggest_threshold_creates_new_tag_without_suggestion():
    uc, embed_svc, tag_repo = _make_use_case(auto_merge=0.95, suggest=0.90)
    existing_tag = TagData(id=uuid.uuid4(), name="tag-a", tag_group_name="g")
    new_tag = TagData(id=uuid.uuid4(), name="tag a", tag_group_name="g")
...
@pytest.mark.asyncio
async def test_multi_group_tags_compared_independently():
    uc, embed_svc, tag_repo = _make_use_case(auto_merge=0.95, suggest=0.90)
...
@pytest.mark.asyncio
async def test_handler_calls_commit_on_success():
    ...
    handler = TagNormalizationHandler(use_case=uc, event_bus=bus, session=session)
...
@pytest.mark.asyncio
async def test_handler_passes_tag_groups_to_use_case():
    from src.modules.intelligence.application.event_handlers.tag_normalization_handler import (
        TagNormalizationHandler,
    )
    from src.modules.intelligence.application.use_cases.normalize_tags import NormalizeTagsResult
