...
def test_find_by_article_id_returns_none_when_not_found():
    from src.infrastructure.persistence.collection.arxiv_metadata_repo_impl import (
...
def test_find_by_article_id_maps_row_to_entity():
    ...
    assert isinstance(entity, ArxivMetadata)
...
def test_save_flushes_and_returns_entity():
    ...
    assert isinstance(result, ArxivMetadata)