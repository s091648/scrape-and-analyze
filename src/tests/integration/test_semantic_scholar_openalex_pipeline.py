...
024-async-pipeline-refactor: ProcessScrapedArticleUseCase was converted to
async in place — this file now wires the async repository siblings and
awaits `.execute()`.
"""
import uuid
...
def _wire_process_use_case(async_db_session):
    ...
    metrics_repo = AsyncSqlAlchemyArticleMetricsRepository(session=async_db_session)
...
@pytest.mark.asyncio
async def test_openalex_fetch_persists_doi_arxiv_id_and_seeds_citation_count(async_db_session):
    ...
    use_case = _wire_process_use_case(async_db_session)
    outcome, saved = await use_case.execute(event)

    from src.modules.collection.application.use_cases import ArticleOutcome
    assert outcome == ArticleOutcome.NEW
    assert saved is not None

    article = (await async_db_session.execute(select(Article).filter_by(id=saved.id))).scalars().first()
    ...
    citation_row = (await async_db_session.execute(
        select(ArticleMetricValue).filter_by(article_id=article.id, metric_key="citation_count")
    )).scalars().first()
...
@pytest.mark.asyncio
async def test_openalex_fetch_creates_article_metrics_row_even_without_citation_count(async_db_session):
    ...
    use_case = _wire_process_use_case(async_db_session)
    _, saved = await use_case.execute(event)

    metrics = (await async_db_session.execute(
        select(ArticleMetrics).filter_by(article_id=saved.id)
    )).scalars().first()
...
@pytest.mark.asyncio
async def test_semantic_scholar_fetch_persists_doi_and_seeds_citation_count(async_db_session):
    from models.article import Article
    from models.article_metric_value import ArticleMetricValue
...
@pytest.mark.asyncio
async def test_duplicate_url_from_second_discover_run_is_not_re_saved(async_db_session):
    from models.article import Article
    ...
    await use_case.execute(event_first)
    ...
    outcome, existing = await use_case.execute(event_second)

    from src.modules.collection.application.use_cases import ArticleOutcome
    assert outcome == ArticleOutcome.DUPLICATE_NEEDS_ANALYSIS
    assert existing is not None
    count = (await async_db_session.execute(select(Article).filter_by(url=job.url))).scalars().all()
    assert len(count) == 1
