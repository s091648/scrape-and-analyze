...
find_recent_failures is still used by monitoring (now in SqlAlchemyFailedTaskRepository).
"""
import pytest
import uuid
...
def _make_repo(db_session):
    ...
    return SqlAlchemyFailedTaskRepository(session=db_session)
...
def test_find_recent_failures_returns_unresolved_failures(db_session):
    ...
    recent_ids = {f.id for f in repo.find_recent_failures(hours=24)}
...
def test_failed_task_can_be_resolved(db_session):
    ...
    failure = _insert_failure(db_session, resolved=False)
    assert failure.id in {f.id for f in repo.find_recent_failures(hours=24)}

    failure.resolved = True
    failure.resolved_at = datetime.now(timezone.utc)
    db_session.commit()

    assert failure.id not in {f.id for f in repo.find_recent_failures(hours=24)}
