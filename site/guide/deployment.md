...
| `weekly-report` | `src/`（與 `scrape-and-analyze` 共用程式碼與 `src/Dockerfile`；自己的 `src/railway-weekly-report.toml` 覆寫 startCommand／cronSchedule，UV_GROUP 由 Terraform 管） | 同樣依賴 `src-unit-test` + `src-integration-test`，由 `deploy-staging-weekly-report` 部署 | 無條件部署 |
| `refresh-metrics` | `src/`（同上，自己的 `src/railway-refresh-metrics.toml`） | 同樣依賴 `src-unit-test` + `src-integration-test`，由 `deploy-staging-refresh-metrics` 部署 | 無條件部署 |
